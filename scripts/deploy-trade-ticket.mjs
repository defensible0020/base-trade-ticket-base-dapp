...
const source = await readFile(resolve(process.cwd(), "contracts/BaseTradeTicket.sol"), "utf8");
const input = {
  language: "Solidity",
  sources: {
    "BaseTradeTicket.sol": { content: source },
  },
  settings: {
    outputSelection: {
      "*": {
...
const contract = output.contracts?.["BaseTradeTicket.sol"]?.BaseTradeTicket;
if (!contract) {
...
console.log(`Deploying BaseTradeTicket from ${account.address} on Base...`);
...
console.log("Set NEXT_PUBLIC_TRADE_TICKET_CONTRACT_ADDRESS to this contract address in Vercel.");
